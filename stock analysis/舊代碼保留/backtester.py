...
SENTIMENT_FEATURES  = ['sentiment_score','sentiment_3m_avg','sentiment_trend',
                       'pos_count','neg_count','article_count']
ALL_FEATURES   = TECH_FEATURES + CHIP_FEATURES + FUND_FEATURES + MACRO_FEATURES + STOCK_TYPE_FEATURES + REV_MOM_FEATURES + VALUATION_FEATURES + SENTIMENT_FEATURES
...
def build_features(df: pd.DataFrame, code: str) -> pd.DataFrame:
    ...
    if 'RSP' not in d.columns:
        d['RSP'] = 0.0
    # 產業訊號（資料庫有則直接用，否則補 0）
    for col in ['sentiment_score','sentiment_3m_avg','sentiment_trend',
                'pos_count','neg_count','article_count']:
