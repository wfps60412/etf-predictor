...
def main():
    ...
    # 路徑解析：優先找腳本旁邊的檔案，找不到才用 DATA_DIR
    ...
    OUTPUT  = _find_file('ETF回測報告_v2.xlsx')
    json_p  = _find_file('etf_model_power.json')
    ...
    print(f"\n✅ 資料載入完成，共 {len(all_dfs)} 支 ETF")
    print(f"⏱️  預估時間：每支 ETF 約 5~15 分鐘（Optuna 超參數調優）\n")
    ...
    for code, df_feat in all_dfs.items():
        ...
        display   = f"{code} {name}"
        etf_idx   = list(all_dfs.keys()).index(code) + 1
        etf_total = len(all_dfs)
        print(f"\n{'='*55}")
        print(f"🔄 [{etf_idx}/{etf_total}] {display}  [{category}，{len(feat_cols)}特徵]")
        print(f"{'='*55}")
        ...
        # 短歷史ETF（<3年）改用全歷史訓練，WFV 樣本不足無意義
        MIN_WFV_ROWS = 252 * 3   # 至少3年資料才用 WFV
        use_wfv = len(df_feat) >= MIN_WFV_ROWS
        ...
        for hz_label, hz_days in HORIZONS.items():
            ...
            if use_wfv:
                ...
                windows = range(test_start, len(df_feat) - hz_days, 21)
            else:
                # ── 全歷史模式（新上市ETF）──────────────────────
                test_start = int(len(df_feat) * 0.6)
                windows = range(test_start, len(df_feat) - hz_days, 21)
                print(f"   ⚠️  {code} 歷史不足({len(df_feat)}筆)，改用全歷史訓練")
            ...
            for i in tqdm(windows,
                          ...
                          unit="w",
                          ncols=72,
                          bar_format="{desc}: {bar} {n}/{total} [{elapsed}<{remaining}, {rate_fmt}]"):

                # 訓練集：其他9支 ETF（WFV: 前i筆；全歷史: 全部）
                Xs_l, ys_l = [], []
                train_dfs = [d.iloc[:i] for d in others] if use_wfv else others
                for td in train_dfs:
                    td_c = td.copy()
    ...
    json_path = json_p  # 路徑已在 main() 開頭定義
    with open(json_path, 'w', encoding='utf-8') as f:
        json.dump(power_json, f, ensure_ascii=False, indent=2)
    print(f"📊 etf_model_power.json 已更新（{len(power_json)} 支）→ {json_path}")

    # ── 上傳到 Google Drive ───────────────────────────────────────
    try:
        sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))
        from drive_sync import upload_file_path
        upload_file_path(os.path.abspath(OUTPUT), "ETF回測報告_v2.xlsx")
        upload_file_path(os.path.abspath(json_path), "etf_model_power.json")
    except ImportError:
